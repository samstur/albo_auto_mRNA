...
sigresults <- read.csv("MvA_sig.gene.csv")

allresults <- read.csv("MvA_allgenes.csv")
head(allresults)

deg_normalizedreadcounts <- merge(sigresults,dds_counts,by=c("gene"))
allgene_normalizedreadcounts <- merge(allresults,dds_counts,by=c("gene"))
View(allgene_normalizedreadcounts)
write.csv(deg_normalizedreadcounts,file="MANvAUTO_sigDEG_with_NormalizedCounts.csv",
          row.names = F)
...
pca_plot <- ggplot(PCA_data, aes(x = PC1, y = PC2, color = condition, position_nudge(y=10))) +
  ...
  scale_color_manual(values=c("dark blue", "dark red"),labels=c("AUTO","NON-AUTO-LAB"))+
...
View(sig_res)
length(unique(sig_res$gene)) #793 if MvA. 775 if AvM
...
## compare DEG lists when you flip the reference condition
deg_AvM <- read.csv(file="AvM_sig.gene.csv")
deg_MvA <- read.csv(file="MvA_sig.gene.csv")

length(deg_AvM$gene) #775
length(deg_MvA$gene) #793

length(setdiff(deg_AvM$gene,deg_MvA$gene))
length(setdiff(deg_MvA$gene,deg_AvM$gene))


# Save my workspace to complete_image.RData in th,e
#  data folder of my working directory
setwd("/Users/samsturiale/Documents/Georgetown/Armbruster Lab/Auto_Biting_RNASeq/R_output_MvA")
save.image(file = "MvA_DESeq_results.RData")
# Load objects into my workspace
setwd("/Users/samsturiale/Documents/Georgetown/Armbruster Lab/Auto_Biting_RNASeq/R_output_MvA")
load(file = "MvA_DESeq_results.RData")
